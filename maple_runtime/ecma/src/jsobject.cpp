...
#define ADD_ACCESSOR_PROPERTY(builtin_property_name_id, builtin_obj_id, getter, get_attrs, setter, set_attrs, attrs) \
  ...
  switch (obj->builtin_id) {
    ...
    case JSBUILTIN_NUMBERPROTOTYPE:
      ...
      ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_TO_LOCALE_STRING_UL, __jsnum_pt_toLocaleString, ATTRS(UNCERTAIN_NARGS, 0));
      ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_VALUE_OF_UL, __jsnum_pt_valueOf, ATTRS(0, 0));
    ...
    case JSBUILTIN_INTL_COLLATOR_CONSTRUCTOR:
      ...
      ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_SUPPORTED_LOCALES_OF, __jsintl_CollatorSupportedLocalesOf, ATTRS(UNCERTAIN_NARGS, 1));
      break;
    ...
    case JSBUILTIN_INTL_NUMBERFORMAT_CONSTRUCTOR:
      ...
      ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_SUPPORTED_LOCALES_OF, __jsintl_NumberFormatSupportedLocalesOf, ATTRS(UNCERTAIN_NARGS, 1));
      break;
    ...
    case JSBUILTIN_INTL_DATETIMEFORMAT_CONSTRUCTOR:
      ...
      ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_SUPPORTED_LOCALES_OF, __jsintl_DateTimeFormatSupportedLocalesOf, ATTRS(UNCERTAIN_NARGS, 1));
      break;
    case JSBUILTIN_INTL_DATETIMEFORMAT_PROTOTYPE:
      ...
      //ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_FORMAT, __jsintl_DateTimeFormatFormat, ATTRS(1, 0));
    ...
    case JSBUILTIN_CONSOLE:
      ADD_FUNCTION_PROPERTY(JSBUILTIN_STRING_LOG, __jsconsole_pt_log, ATTRS(UNCERTAIN_NARGS, 2));
      break;
...
// ecma 8.12.5
bool __jsobj_internal_Put(__jsobject *o, __jsstring *p, TValue &v, bool throw_p, bool isStrict) {
  bool isNum;
  uint32 idxNum = __jsstr_is_numidx(p, isNum);
  if (isNum) {
...
bool __jsop_setprop(TValue &o, TValue &p, TValue &v) {
  ...
  if (__is_number(p) && __jsval_to_number(p) >= 0) {
    ...
    __jsstring *name = __js_ToString(p);
    rt = __jsobj_internal_Put(obj, name, v, false);
    if (!__is_string(p)) {
      memory_manager->RecallString(name);
    }
  }
...
TValue __jsop_getprop_by_name(TValue &o, __jsstring *p) {
  ...
  if (__is_js_object(o)) {
    ...
    TValue retV = __jsobj_internal_Get(obj, p);
    if (__is_js_function(retV)) {
      __js_function_setup_this_object(&retV, obj);
    }
    return retV;
...
bool __jsop_setprop_by_name(TValue &o, __jsstring *p, TValue &v, bool isStrict) {
  ...
  rt = __jsobj_internal_Put(obj, p, v, false, isStrict);
  if (!__is_js_object(o)) {
    memory_manager->ManageObject(obj, RECALL);
  }
  return rt;
...
void __jsconsole_pt_log (TValue &thisArg, TValue *argList, uint32_t nargs) {
  for (uint32_t i = 0; i < nargs; i++) {
    __jsop_print_item(argList[i]);
    if (i != nargs - 1) {
      printf(" ");
    }
  }
