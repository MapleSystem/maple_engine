...
__jsobject *__js_ToRegExp(__jsstring *jsstr) {
  ...
  const char slash[] = "/";
  TValue search = __string_value(__jsstr_new_from_char(slash));
  TValue pos = __number_value(0);
  TValue str = __string_value(js_pattern_flags);
  ...
  if (__js_ToNumber(pos) >= len) {
    ...
    TValue flags_val = __jsstr_substring(str, pos, end);
  ...
  TValue js_ignorecase = __boolean_value(ignorecase);
  ...
  if (nargs == 0) {
    ...
    if (__is_undefined(arg_list[0])) {
      ...
      if (arg_obj->object_class == JSREGEXP) {
        ...
        TValue jsval = __js_ToPrimitive(arg_list[0], JSTYPE_STRING);
    ...
    if (__is_undefined(arg_list[1])) {
      ...
      TValue jsval = __js_ToPrimitive(arg_list[1], JSTYPE_STRING);
...
TValue __jsregexp_Exec(TValue &this_value, TValue &value) {
  ...
  TValue source = __jsop_getprop_by_name(this_value,
                        __jsstr_get_builtin(JSBUILTIN_STRING_SOURCE));
  ...
  if (__is_js_object(value)) {
    TValue jsval = __js_ToPrimitive(value, JSTYPE_STRING);
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (start >= 0 && end - start >= 0) {
      TValue subject_val = __string_value(js_subject);
...
TValue __jsregexp_Test(TValue &this_value, TValue &value) {
  ...
  TValue match = __jsregexp_Exec(this_value, value);
  if (__is_null(match)) {
